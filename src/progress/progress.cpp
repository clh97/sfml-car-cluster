    ...
    : min(min), max(max), value(0), radius(radius), m_position(position)
{
    ...
    m_background.setOrigin(sf::Vector2f(m_background.getRadius(), m_background.getRadius()));
...
void Progress::draw(sf::RenderTarget &target, const sf::RenderStates &states) const
{
    target.draw(m_background);
    // target.draw(&m_vertexArray[0], m_vertexArray.getVertexCount());
...
void Progress::update(sf::Time elapsed)
{
    ...
    this->m_vertexArray = sf::VertexArray(sf::PrimitiveType::TriangleFan, numPoints + 2);
