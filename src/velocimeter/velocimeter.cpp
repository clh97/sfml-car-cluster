...
void Velocimeter::update(sf::Time elapsed)
{
    ...
    m_text.setOrigin(sf::Vector2f(m_text.getLocalBounds().width / 2.f, m_text.getLocalBounds().height / 2.f));
    ...
    for (int i = (int)min; i <= max; i += max >= 1000 ? 1000 : 10)
    {
        ...
        line.setOrigin(sf::Vector2f(1.f, this->dialRadius * 0.9f));
        line.setFillColor(tickColor);
        line.setOutlineColor(tickColor);
        line.setOutlineThickness(tickWidth);
        line.setRotation(sf::degrees(angle));
        line.setPosition(sf::Vector2f(m_dial.getPosition().x, m_dial.getPosition().y));
...
void Velocimeter::setValue(float value)
{
    ...
    int rotation = m_needle.getRotation().asDegrees();
    ...
    m_needle.setRotation(sf::degrees(interpolatedAngle));
...
void Velocimeter::draw(sf::RenderTarget &target, const sf::RenderStates &states) const
{
    target.draw(m_dial);
    ...
    : m_dial(), m_needle_center(), m_needle(), m_text(), min(min), max(max), dialRadius(size.x), textSuffix(textSuffix), m_background()
{
    ...
    m_dial.setOrigin(sf::Vector2f(this->dialRadius, this->dialRadius));
    m_dial.setPosition(sf::Vector2f(position.x + size.x / 2, position.y + size.y / 2));

    m_needle.setSize(sf::Vector2f(needleLength, 6.f));
    m_needle.setFillColor(sf::Color::Red);
    m_needle.setOrigin(sf::Vector2f(needleLength, 0.0f));
    m_needle.setPosition(sf::Vector2f(position.x + size.x / 2, position.y + size.y / 2));
    m_needle.setRotation(sf::degrees(45.f));
    ...
    m_needle_center.setOrigin(sf::Vector2f(this->dialRadius * 0.7f, this->dialRadius * 0.7f));
    m_needle_center.setPosition(sf::Vector2f(position.x + size.x / 2, position.y + size.y / 2));

    m_text.setFont(font);
    m_text.setFillColor(sf::Color::White);
    m_text.setOrigin(sf::Vector2f(m_text.getLocalBounds().width / 2.f, m_text.getLocalBounds().height / 2.f));
    m_text.setPosition(sf::Vector2f(position.x + size.x / 2, position.y + size.y / 2));
    ...
    m_background.setOrigin(sf::Vector2f(this->dialRadius, this->dialRadius));
    m_background.setPosition(sf::Vector2f(position.x + size.x / 2, position.y + size.y / 2));
