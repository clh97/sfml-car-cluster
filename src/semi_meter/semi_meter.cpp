...
void SemiMeter::update(sf::Time elapsed)
{
    ...
    for (int i = (int)min; i <= max; i += max >= 1000 ? 1000 : 10)
    {
        ...
        line.setPosition(sf::Vector2f(this->position.x, this->position.y));
...
void SemiMeter::draw(sf::RenderTarget &target, const sf::RenderStates &states) const
{
    target.draw(m_dial);
    ...
    : m_dial(), m_needle_center(), m_needle(), m_text(), min(min), max(max), dialRadius(size.x), textSuffix(textSuffix), position(position), size(size)
{
    ...
    // m_dial.setRotation(sf::degrees(90.f));

    m_needle.setSize(sf::Vector2f(needleLength, 6.f));
    m_needle.setFillColor(sf::Color::Red);
    m_needle.setOrigin(sf::Vector2f(needleLength, 0.0f));
    m_needle.setPosition(sf::Vector2f(position.x, position.y));
    m_needle.setRotation(sf::degrees(90.f));
    ...
    m_needle_center.setOrigin(sf::Vector2f(this->dialRadius * 0.25f, this->dialRadius * 0.25f));
    m_needle_center.setPosition(sf::Vector2f(position.x, position.y));
